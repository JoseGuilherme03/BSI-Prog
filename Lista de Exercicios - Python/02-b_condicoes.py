...
def aumento_salario(salario):
    ...
    if salario <= salario_minimo:  # até 1 SM
        novo_salario = salario + (salario * 20) / 100

    elif salario <= salario_minimo * 2:  # de 1 até 2 SM
        novo_salario = salario + (salario * 15) / 100

    elif salario <= salario_minimo * 5:  # de 2 até 5
...
def converte_hora_24_para_12(horario):
    ...
    elif int(horas) >= 1 and int(horas) <= 11:
        if horas[0] == "0":
            return horas[1] + ":" + minutos + " am"
        else:
            return horario + " am"

    elif int(horas) >= 13 and int(horas) <= 23:
        horas = int(horas) - 12
        return str(horas) + ":" + minutos + " pm"
...
def converte_hora_12_para_24(horario):
    ...
    if int(horas) >= 1 and int(horas) <= 11 and "am" in minutos:
        if len(horas) == 1:
            return "0" + horas + ":" + minutos[0:2]
        else:
            return horas + ":" + minutos[0:2]

    elif horas == "12" and "pm" in minutos:
        return horas + ":" + minutos[0:2]
...
def conta_combustivel(qtde_litros, tipo_combustivel, tipo_pagamento):
    ...
    if tipo_combustivel == 'a':
        totvalor = (qtde_litros * alcool)
    elif tipo_combustivel == 'g':
        totvalor = (qtde_litros * gasolina)
    elif tipo_combustivel == 'd':
        totvalor = (qtde_litros * diesel)
    
    if tipo_pagamento == 'v' or tipo_pagamento == 'd':
        return round(totvalor - (totvalor * 10) / 100 , 2)
    else:
        return round(totvalor,2)
...
def comprar_frutas(morango=0, uva=0):
    ...
    totvalor = (uva * preco_uva) + (morango * preco_mor)

    if uva == 0 and morango == 0:
        return 0   
        
    elif qt_total > 8 or totvalor > 25:
        return round(totvalor - (totvalor * 10) / 100 , 2)
