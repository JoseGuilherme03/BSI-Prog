...
def leet(texto):
    ...
    Retorna:
        string: o texto convertido, conforme o enunciado.
    """
    letras = 'aegistoAEGISTO'
    numeros = '43915704391570'
    troca = str.maketrans(letras,numeros)
    return texto.translate(troca)


'''
    troca = {"a": "4", "e": "3", "g": "9", "i": "1", "s": "5", "t": "7", "o": "0",'E':'3','T':'7','O':'0'}
    texto = texto.replace("a", troca["a"])
    texto = texto.replace("e", troca["e"])
    texto = texto.replace("g", troca["g"])
    texto = texto.replace("i", troca["i"])
    texto = texto.replace("s", troca["s"])
    texto = texto.replace("t", troca["t"])
    texto = texto.replace("o", troca["o"])
    texto = texto.replace("E", troca["E"])
    texto = texto.replace("T", troca["T"])
    texto = texto.replace("O", troca["O"])
    return texto
